...
the same transport**, including Apple Floodgate with its corporate CA bundle.
...
and accepts `base_url`, so Floodgate works unchanged. A real call through
`floodgate.g.apple.com` returned 200 OK.
...
def create_chat_model(agent_name: str = "scenario_generator",
    ...
    Mirrors `LLMConfigManager.create_generator()`:
      ...
      * Floodgate or the direct Gemini API, per `LLM_PROVIDER`
    ...
    if provider == "floodgate":
        kwargs.update(_floodgate_kwargs())
    else:
...
def _floodgate_kwargs() -> Dict[str, Any]:
    """
    Point the model at Floodgate, with the corporate CA bundle.
    ...
    CERTIFICATE_VERIFY_FAILED. `config.floodgate.ssl_context()` assembles a
    bundle from the system keychains. Verification is NEVER disabled.
    """
    from config.floodgate import (FLOODGATE_BASE_URL, get_floodgate_token,
                                  ssl_context)

    token = get_floodgate_token()
    if not token:
        raise RuntimeError(
            "LLM_PROVIDER=floodgate but no token could be resolved; "
            "see ./scripts/check_floodgate.py")

    kwargs: Dict[str, Any] = {
        # api_key is required by the constructor but unused on this route: the
        # gateway authenticates from the Authorization header.
        "api_key": "unused-floodgate-route",
    ...
    try:
        ...
        logger.warning(f"⚠️ Could not build the Floodgate CA bundle: {e}")
    return kwargs
