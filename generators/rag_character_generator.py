...
# Gemini imports for LLM generation
try:
...
class CharacterDetails:
    ...
    # Cosmere/Roshar-specific attributes (compatible with character_manager.py)
    ...
    def __post_init__(self):
        ...
        if self.features is None:
            self.features = []
        if self.investiture_points is None:
            self.investiture_points = {"current": 0, "maximum": 0}
        if self.spren is None:
            self.spren = {"type": None, "name": None, "status": "dormant"}
        if self.surges_known is None:
            self.surges_known = []
        if self.cantrips_known is None:
            self.cantrips_known = []
        if self.spells_known is None:
            self.spells_known = []
        if self.languages is None:
            self.languages = []
        if self.tool_proficiencies is None:
            self.tool_proficiencies = []
        if self.armor_proficiencies is None:
            self.armor_proficiencies = []
        if self.weapon_proficiencies is None:
            self.weapon_proficiencies = []
        if self.saving_throw_proficiencies is None:
...
class CharacterGenerator:
    ...
    def _initialize_llm(self):
        """Initialize Gemini LLM for content generation"""
        try:
            if GEMINI_AVAILABLE:
                import os
                ...
                self.llm = GeminiChatGenerator(
                    model_name="gemini-2.0-flash",
                    generation_config={"temperature": 0.7}
                )
                if self.verbose:
                    print("✓ Gemini LLM initialized for content generation")
            else:
                self.llm = None
                if self.verbose:
                    print("⚠️  Gemini not available, using fallback content generation")
    ...
    def generate_with_llm(self, prompt: str, context: str = "") -> str:
        """Generate content using Gemini LLM with context"""
        if not self.llm:
...
Generate detailed, specific content that fits D&D 5e rules. Be creative but accurate to the game's mechanics and lore."""
            ...
            if result and "replies" in result and result["replies"]:
                # Gemini returns text in the text attribute
...
Radiant Order: {character.radiant_order or 'None'}
Ideal Level: {character.ideal_level} ({self._get_ideal_name(character.ideal_level)})
Investiture Points: {character.investiture_points.get('current', 0)}/{character.investiture_points.get('maximum', 0)}
Spren Bond: {character.spren.get('type', 'None')} - {character.spren.get('name', 'Unnamed')} ({character.spren.get('status', 'dormant')})
Surges Known: {', '.join(character.surges_known) if character.surges_known else 'None'}
Cantrips Known: {', '.join(character.cantrips_known) if character.cantrips_known else 'None'}
Invested Arts Known: {', '.join(character.spells_known) if character.spells_known else 'None'}
Languages: {', '.join(character.languages) if character.languages else 'Common'}

PROFICIENCIES:
Tool Proficiencies: {', '.join(character.tool_proficiencies) if character.tool_proficiencies else 'None'}
Armor Proficiencies: {', '.join(character.armor_proficiencies) if character.armor_proficiencies else 'None'}
Weapon Proficiencies: {', '.join(character.weapon_proficiencies) if character.weapon_proficiencies else 'None'}
Saving Throw Proficiencies: {', '.join(character.saving_throw_proficiencies) if character.saving_throw_proficiencies else 'None'}
...
This is a level {character.level} {character.identity} {character.character_class} with {character.background} background.
...
Radiant progression: {self._get_ideal_name(character.ideal_level)} Ideal, {character.radiant_order or 'No Order'}, {character.investiture_points.get('current', 0)} investiture points.

TAGS FOR INDEXING:
Character, {character.identity}, {character.race}, {character.character_class}, {character.background}, {character.radiant_order or 'NonRadiant'}, Level{character.level}, Ideal{character.ideal_level}, {character.rulebook}"""
