...
**Goal:** Create NPC stat generation system using LLM + RAG

**This phase is UNCHANGED from previous plan - it's still valid.**

#### Deliverables

**1. components/combat/npc_stat_generator.py** (~400 lines)

```python
class NPCStatGenerator:
    """
    Generates D&D 5e NPC stats using LLM + RAG.

    Process:
    1. Parse scenario text for enemy descriptions
    2. Query RAG for similar creature stats
    3. Use LLM to generate complete D&D stat block
    4. Validate and repair stats
    5. Return NPC data dict
    """
    ...
    def generate_npc_stats(
        ...
        """
        Generate NPC stats via LLM with RAG.
...
Output JSON with:
{
    "name": "NPC Name",
    "level": 1-20,
    "character_class": "Fighter|Rogue|Wizard|etc",
    "race": "Goblin|Human|etc",
    "background": "Soldier|Criminal|etc",
    "ability_scores": {
        "strength": 1-30,
        "dexterity": 1-30,
        "constitution": 1-30,
    ...
    "hit_points": {"maximum": X, "current": X, "temporary": 0},
    "armor_class": 8-25,
...
Role: {role}
Party Level: {context.get('party_level', 1)}
...
Generate complete stat block:"""
        ...
        # Step 4: Parse JSON
        npc_data = self._parse_json_response(response.content)

        # Step 5: Validate and repair
        npc_data = self.validate_and_repair(npc_data, challenge_rating)

        logger.info(f"✅ Generated NPC: {npc_data['name']} (AC {npc_data['armor_class']}, HP {npc_data['hit_points']['maximum']})")

        return npc_data

    def validate_and_repair(self, npc_data: Dict, target_cr: float) -> Dict:
        """
        Validate NPC stats against D&D 5e rules.

        Checks:
        - All ability scores 1-30
        - HP calculated correctly
        - AC reasonable for CR (8-20)
        - Attack bonus matches proficiency + ability mod
        - Proficiency bonus correct for level/CR

        Repairs:
        - Clamps ability scores to 1-30
        - Recalculates HP if wrong
        - Sets default AC if missing (10 + DEX mod)
        - Fills missing attacks with "Unarmed Strike"
        - Ensures all required fields present
        """
        # Validate ability scores
        for ability, score in npc_data.get("ability_scores", {}).items():
            if score < 1:
                npc_data["ability_scores"][ability] = 1
                logger.warning(f"Clamped {ability} to minimum 1")
            elif score > 30:
                npc_data["ability_scores"][ability] = 30
                logger.warning(f"Clamped {ability} to maximum 30")

        # Validate HP
        max_hp = npc_data.get("hit_points", {}).get("maximum", 0)
        if max_hp <= 0:
            # Calculate based on level and CON
            ...
            npc_data["hit_points"] = {"maximum": max_hp, "current": max_hp}
            logger.warning(f"Recalculated HP: {max_hp}")

        # Validate AC
        if "armor_class" not in npc_data or npc_data["armor_class"] < 8:
            dex_mod = (npc_data["ability_scores"]["dexterity"] - 10) // 2
            npc_data["armor_class"] = 10 + dex_mod
            logger.warning(f"Set default AC: {npc_data['armor_class']}")
        ...
        if not npc_data.get("attacks"):
            ...
            logger.warning("Added default unarmed strike")

        # Ensure required fields
        npc_data.setdefault("name", "Unknown NPC")
        npc_data.setdefault("level", 1)
        npc_data.setdefault("class", "Warrior")
    ...
    def get_npc_from_template(self, template_name: str) -> Dict:
        """
        ...
        Returns:
            NPC stat dict from templates file
        """
    ...
    def _load_templates(self) -> Dict[str, Dict]:
        ...
        if template_path.exists():
            ...
            logger.info(f"Loaded {len(templates)} NPC templates")
            return templates
        else:
            logger.warning("NPC templates file not found, returning empty dict")
            return {}
    ...
    def _parse_json_response(self, response: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Failed to parse NPC JSON: {e}")
...
```python
# In components/character_manager.py

class CharacterManager:
    def __init__(self):
        self.characters: Dict[str, CharacterData] = {}
        self._npc_counter = 1  # NEW: Track NPC IDs
    ...
    def add_npc(self, npc_data: Dict[str, Any]) -> str:
        ...
        Args:
            npc_data: Dict with D&D stats from NPCStatGenerator

        Returns:
            char_id: Unique ID for NPC (e.g., "goblin_001")
        """
        # Generate unique ID
        base_name = npc_data['name'].lower().replace(' ', '_')
        char_id = f"{base_name}_{self._npc_counter:03d}"
        self._npc_counter += 1
        ...
        character = CharacterData(
            name=npc_data["name"],
            level=npc_data.get("level", 1),
            char_class=npc_data.get("class", "Warrior"),
            ...
            skill_proficiencies=[],  # NPCs typically don't have skill proficiencies tracked
            armor_class=npc_data["armor_class"],
            hit_points=npc_data["hit_points"]["current"],
            max_hit_points=npc_data["hit_points"]["maximum"],
            # Roshar extensions (empty for non-Radiant NPCs)
            radiant_order="",
            ideal_level=0,
        ...
        self.logger.info(f"Added NPC: {char_id} ({npc_data['name']}, CR {character.challenge_rating})")

        return char_id

    def remove_npc(self, char_id: str):
        ...
        if char_id in self.characters:
            ...
            self.logger.info(f"Removed NPC: {char_id} ({npc_name})")

    def get_npcs(self) -> List[str]:
        """Return list of NPC char_ids (for cleanup after combat)"""
        # NPCs have numeric suffixes like "_001", "_002"
        import re
