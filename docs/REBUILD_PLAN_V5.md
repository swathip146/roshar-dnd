...
| # | Fix | File |
|---|---|---|
| 0.12 | **Wire up `DocumentSplitter`.** It is imported at line 23 and **never used** — Docling output goes straight to the embedder, which is why chunks run from tiny to **34,586 chars** (median 1,404, p90 5,017). A 34k-char chunk embeds to a meaningless average and can never retrieve precisely. Insert `DocumentSplitter(split_by="word", split_length=200, split_overlap=30)` in `store_in_qdrant()` before embedding. | `generators/batch_qdrant_indexer.py:23,94` |
...
| 2.9 | **Three-tier rules architecture** (§8, §9c). Tier 1: vendor `5e-bits/5e-database` JSON for baseline SRD mechanics. Tier 2: hand-author `data/rules/stormlight/*.json` for the Surgebinding hot path (~20-40 entries, 1-2 days). Tier 3: Qdrant for narrative lore only. **Rules are adjudicated from Tiers 1-2; the vector DB never adjudicates.** |
