...
**Exit criteria:** save → quit → load preserves HP, inventory, location, and quests. `pytest tests/` completes without INTERNALERROR. Verification script (§10) shows Radiant Handbook chunks > 0 and prose chunks bounded. RAG text is visibly present in the scenario prompt. **✅ All met** — see §14a, which also restates the chunk-size criterion: it originally said "max chunk < ~2,000 chars", which is right for prose and wrong for markdown tables, since 0.12 deliberately keeps those whole so headers stay attached to their rows. Read it as **p99 of prose chunks < 2,000** (measured: 1,938).
...
```bash
LLM_PROVIDER=floodgate ./scripts/playtest.py --turns 3     # the standing gate
...
| 2.10 | **Rules long tail** | ✅ **the real defect is fixed; the extraction script is closed WON'T-DO** | **The tail was never missing DATA — it was unreachable data.** `SRDRules` loaded ten datasets (1,321 entries) from the start, but `query_rules` searched only FOUR (monsters/spells/conditions/equipment). So 386 entries across six datasets were loaded and unsearchable, and a lookup for "Perception" — plainly in `skills.json` — fell through to the LLM rules judge. `data/rules/gaps.json` recorded it happening: `"rules lookup perception"`, tier `judged`. Fixed by adding the six missing accessors + `SRDRules.lookup()` and pointing `query_rules` at it (2026-09-11). Two matcher bugs surfaced with it: a FUZZY hit in `magic_items` beat the EXACT hit in `damage_types` so "Necrotic" resolved to "Potion of Necrotic Resistance" (`lookup()` now makes two passes, exact-everywhere then fuzzy); and `ability_scores.json` keys entries `"STR"`/`"DEX"` with the searchable word in `full_name`, which `_find` never consulted, so "Dexterity" found nothing. **The LLM-extraction script is deliberately NOT built:** `surgebinding.json` is already hand-extracted, `"reviewed": true`, with line-number citations that `verify_citations()` checks. Regenerating correct, cited data through a lossier process is a regression risk, not a gap — and §13's own note (line ~1089) already said 2.10 must have a stopping point rather than structuring all 299 pages. D5's promotion path handles genuine long-tail misses as play surfaces them. `tests/test_conditions_and_rules_coverage.py` (91 tests) |
...
the Floodgate CA route. It does not — it uses `from google import genai`, the same
SDK Phase 4 ported to, and accepts `base_url`. Three real calls through
`floodgate.g.apple.com` returned 200 OK (plain text, structured output, tool
...
| `agents/langgraph_models.py` | LangChain chat models over the same transport, Floodgate included |
...
ff6fca4  Add the LangGraph agent-layer seam, proven over Floodgate
...
| `langchain-google-genai` uses the deprecated SDK and will break Floodgate | It uses `from google import genai` — the same SDK Phase 4 ported to — and accepts `base_url`. Three real 200 OKs through the gateway |
...
Proven live over Floodgate after the fix: the model calls `get_world_state` **once**,
