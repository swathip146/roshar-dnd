...
|---|---|---|
| LLM calls | **none** — a scripted fake generator | real Gemini/Floodgate calls |
...
./scripts/playtest.py --provider floodgate             # NEW: transport parity
...
| L8 | A degraded-LLM run (forced HTTP 500) still produces legal turns — the failure that motivated Floodgate |
