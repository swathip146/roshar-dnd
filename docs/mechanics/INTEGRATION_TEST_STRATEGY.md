...
| E3 | Encumbrance | Speed penalty in combat **and** disadvantage on STR/DEX/CON checks | ✅ PASS | `exploration_full` | 2 checks: {'encumbrance_level': 'normal', 'speed_penalty': 0, 'has_di... | 🟡 | reached indirectly via `get_character_state`. **Suite A's own E3 test was weak and hid a real bug** — it asserted only `isinstance(state, dict)` on an UNENCUMBERED character, while `over_capacity` returned `speed_penalty: 0, has_disadvantage: False`, making 724 lb strictly better than 101 lb. Fixed and now pinned by a monotonicity assertion plus end-to-end speed/disadvantage checks. |
...
| R4 | Cantrips are free | IP unchanged after a cantrip — guards the "charged for a free ability" regression | ✅ PASS | `cosmere_full` | via `G5:cantrips_free` + `G5:cantrip_free_at_ledger`: 10 surges all cost 0; cantrip free with an empty pool | ✅ | **verified end-to-end through the resolver**: `illumination`, `lashing` and `soulcast` each leave IP and Stormlight untouched, while the costed `progression_healing` charges 2 IP — so the test can tell free from costed. Note established by sabotage: combat never DEDUCTS Stormlight at all (`stormlight_cost` is only an affordability gate at `action_registry.py:647`; the one deduction is `character_manager.py:2810` on the exploration path), so a wrong cantrip cost is invisible at runtime and is pinned by a separate metadata assertion instead. |
