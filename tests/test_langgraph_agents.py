...
separately (a real call through floodgate.g.apple.com returned 200 OK for plain
...
class TestTheDependenciesArePresent:
    ...
    def test_langchain_uses_the_current_google_sdk(self):
        ...
        pulled the old SDK back in, Floodgate's transport and CA bundle would not
    ...
    def test_the_model_accepts_a_custom_base_url(self):
        """Floodgate is reachable only if base_url is supported."""
        import inspect
        ...
        assert "additional_headers" in parameters, (
            "no way to send the Floodgate bearer token")
...
class TestSchemaAndToolsAreNotCombined:
    ...
    def test_tools_win_and_the_schema_is_dropped(self, monkeypatch):
        ...
        monkeypatch.setattr("langchain_google_genai.ChatGoogleGenerativeAI",
                            _FakeModel)
        monkeypatch.setattr(module, "_floodgate_kwargs", lambda: {"api_key": "x"})
...
class TestTheTransportIsPreserved:
    def test_floodgate_sends_a_bearer_token(self, monkeypatch):
        import agents.langgraph_models as module

        monkeypatch.setattr(module, "_model_name", lambda: "gemini-2.5-flash")
        monkeypatch.setattr("config.floodgate.get_floodgate_token",
                            lambda: "test-token")
        monkeypatch.setattr("config.floodgate.ssl_context", lambda: None)

        kwargs = module._floodgate_kwargs()
        assert kwargs["additional_headers"]["Authorization"] == "Bearer test-token"
        assert "floodgate" in kwargs["base_url"]

    def test_a_missing_token_fails_loudly(self, monkeypatch):
        """Never silently fall back to the direct API — that hides an outage."""
        import agents.langgraph_models as module

        monkeypatch.setattr("config.floodgate.get_floodgate_token", lambda: "")
        with pytest.raises(RuntimeError, match="floodgate"):
            module._floodgate_kwargs()
    ...
    def test_the_provider_switch_is_respected(self, monkeypatch):
        ...
        monkeypatch.setattr("langchain_google_genai.ChatGoogleGenerativeAI",
                            _FakeModel)
        monkeypatch.setattr(module, "_floodgate_kwargs",
                            lambda: {"api_key": "unused", "base_url": "fg"})

        monkeypatch.setattr("config.floodgate.resolve_provider",
                            lambda: "floodgate")
        module.create_chat_model("main_interface")
