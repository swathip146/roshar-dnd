...
@component
class FloodgateChatGenerator:
    """
    Gemini via the Floodgate (hwtgenie) OpenAI-compatible proxy — see
    config/floodgate.py for why and for how credentials are resolved.
    ...
    def __init__(self, model_name: str, generation_config: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            raise ImportError(
                "The Floodgate provider needs the openai package "
                "(pip install openai)") from e

        self.model_name = to_floodgate_model(model_name)
        self.generation_config = dict(generation_config or {})
        self.response_schema = response_schema
        self.client = OpenAI(base_url=FLOODGATE_BASE_URL,
                             api_key=get_floodgate_token())
        logger.info(f"🔀 FloodgateChatGenerator ready ({self.model_name})")
    ...
    def run(self, messages: List[ChatMessage],
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                transient = status in (408, 429, 500, 502, 503, 504)
                if not transient or attempt == attempts:
                    message = f"Floodgate API error: {e}"
                    logger.error(
                        f"❌ FLOODGATE ERROR"
                ...
                delay = self.RETRY_BASE_DELAY_SECONDS * (2 ** (attempt - 1))
                logger.warning(
                    f"⚠️ Floodgate HTTP {status} (attempt {attempt}/{attempts}); "
                    f"retrying in {delay:.1f}s")
                _time.sleep(delay)
        ...
        if not text:
            ...
            if reason and str(reason).lower() not in ("stop", "none"):
                raise GeminiAPIError(
                    f"Floodgate returned no text (finish_reason={reason})")
