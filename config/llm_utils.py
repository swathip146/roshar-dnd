...
@component
class FloodgateChatGenerator(GeminiChatGenerator):
    """
    Gemini via Apple's Floodgate gateway — see config/floodgate.py.

    SUBCLASSES GeminiChatGenerator on purpose. Floodgate at
    floodgate.g.apple.com/api/gemini speaks the NATIVE Gemini API, so everything
    ...
    def __init__(self, model_name: str, generation_config: Optional[Dict[str, Any]] = None,
        ...
        # Floodgate takes a bare model name on this path; the "gcp:" prefix
        ...
        headers = {"Authorization": f"Bearer {get_floodgate_token()}"}
        project_token = (os.getenv(FLOODGATE_PROJECT_TOKEN_ENV) or "").strip()
        if project_token:
        ...
        logger.info(
            f"🔀 FloodgateChatGenerator ready ({self.model_name} via "
            f"{FLOODGATE_BASE_URL})")
