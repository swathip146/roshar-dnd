...
class LLMConfigManager:
    ...
    def _create_gemini_generator(self, config: LLMConfig, response_schema: Optional[Dict[str, Any]] = None) -> Any:
        ...
        #
        # `config/gateway.py` is an untracked, machine-local module that can route the
        # same Gemini models through a corporate proxy. It is deliberately NOT in the
        # repository: it encodes one organisation's internal endpoints and credential
        # flow, which is useless (and inappropriate) elsewhere.
        #
        # Absent, this falls back to the direct Gemini API — the default — so a fresh
        # clone works with nothing but GEMINI_API_KEY. The import is guarded because an
        # unguarded one made generator creation raise ImportError on any machine without
        # the module, which would stop the game from starting at all.
        provider = "gemini"
        try:
            ...
            logger.debug("🔌 No local gateway module; using the direct Gemini API")
        ...
        if provider == "gateway":
            ...
            logger.info(f"🔀 Using the local gateway transport for {config.model}")
            return generator
